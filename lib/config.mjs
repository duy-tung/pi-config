...
const definitions = {
  main: {
    runtime: "current", provider: "anthropic", model: "claude-opus-5-5", thinking: "high",
    theme: "rose-pine-moon", packages: [...common, "@tintinweb/pi-subagents", "pi-lens", "pi-goal-x", "pi-background-tasks", "pi-advisor-flow", ...uiPackages],
  },
};
...
export function buildConfiguration({ root, agentDir, binDir, nodePath, platform = process.platform, home, repoDir, shellPath }) {
  ...
  const permissions = {
    ...
    deny: [
      ...
      "Path(~/.gnupg/**)", "Path(~/.kube/config)", "Path(~/.netrc)", "Path(~/.git-credentials)",
      "Path(~/.config/gh/hosts.yml)", "Path(~/.docker/config.json)", "Path(**/id_rsa*)", "Path(**/id_ed25519*)", "Path(**/*.pem)",
      "Bash(rm -rf *)", "Bash(sudo *)", "Bash(*firecrawl-key.cjs*)",
      "mcpScript",
    ],
  };
  // Một model cho cả hai giai đoạn để giai đoạn 2 dùng lại cache của giai đoạn 1.
  const autoMode = { model: "openai-codex/gpt-6-sol", stage2Reasoning: "low" };
  for (const [name, profile] of Object.entries(profiles)) {
    ...
    const settings = {
      ...
      extensions: ["rose-pine-palette.ts", "pi-rewind", "native-web-search", "claude-usage", "pi-auto-mode"]
    ...
    settings.enabledModels = ["anthropic/claude-opus-5-5", "openai-codex/gpt-6-sol", "openai-codex/gpt-6-astra", "opencode-go/glm-5.3-flash"];
    ...
    // Native search theo model hiện tại: bước "openai" + useCurrentModel dùng hosted web_search của
    // Codex/OpenAI hoặc web search của Claude (cầu nối native-web-search). Model khác (GLM) và lỗi tạm thời
    // đi tiếp sang Exa (không có key thì dùng endpoint MCP miễn phí của Exa), rồi Firecrawl.
    // Không đặt "provider" vì provider cố định sẽ bỏ qua searchRouting.
    addJson(join(dir, "web-search.json"), {
      workflow: "none", allowBrowserCookies: false,
      webSearch: { allowedProviders: ["openai", "exa", "firecrawl"] },
      searchRouting: {
        providers: ["openai", "exa", "firecrawl"], useCurrentModel: true,
