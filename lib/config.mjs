...
export function buildConfiguration({ root, agentDir, binDir, nodePath, platform = process.platform, home, repoDir, shellPath }) {
  ...
  const protectedPaths = [
    ...
    join(root, "secrets", "*.env"), join(root, "backups", "**"), firecrawlCredentials,
  ];
  const policy = {
    permission: {
      ...
      path: {
        ...
        "**/compact-adviser.json": "deny", "**/routing.json": "deny", "**/routing-capabilities.json": "deny", "**/routing-state/**": "deny", "**/routing-writers/**": "deny",
      },
