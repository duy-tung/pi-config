...
const roleNames = ["researcher", "worker", "debugger", "reviewer", "researcher-glm", "worker-glm", "debugger-glm"];
const common = [
...
export function buildConfiguration({ root, agentDir, binDir, nodePath, platform = process.platform, home, repoDir, shellPath }) {
  ...
  for (const [name, profile] of Object.entries(profiles)) {
    ...
    if (name === "advisor") addJson(join(dir, "advisor.json"), readJson("advisor"));
    if (name === "goal") addJson(join(dir, "pi-goal-x-settings.json"), readJson("goal"));
    if (name === "main") addJson(join(dir, "pi-usage.json"), { codexFastMode: true });
    let instructions = readText("AGENTS.md");
    if (!hasAgents) instructions = instructions.replace(/^Dùng Agent với.*$/mu, name === "background"
      ? "Profile này dùng bg_run, bg_status và bg_logs cho công việc nền; không tự bật Fusion hoặc tác vụ model trả phí."
      : "Profile này dùng ask_advisor cho ý kiến độc lập khi cần; executor Sol/high, advisor Astra/high. Không tự bật alwaysOn.");
    addText(join(dir, "AGENTS.md"), instructions);
