...
export function buildConfiguration({ root, agentDir, binDir, nodePath, platform = process.platform, home, repoDir, shellPath, modelRoles }) {
  ...
  for (const [name, profile] of Object.entries(profiles)) {
    ...
    // Follow-up thêm Ctrl+Enter: terminal của Orca (xterm.js) gửi Alt+Enter dạng ESC CR, mà Pi khi bật kitty keyboard
    // hiểu là Shift+Enter. Phím mặc định giữ như Pi: Ctrl+Q trên Windows/WSL, Alt+Enter ở nơi khác.
    const windowsKeys = platform === "win32" || (platform === "linux" && Boolean(process.env.WSL_DISTRO_NAME || process.env.WSL_INTEROP));
    const followUp = [windowsKeys ? "ctrl+q" : "alt+enter", "ctrl+enter"];
    addJson(join(dir, "keybindings.json"), { "app.clipboard.pasteImage": [], "app.thinking.cycle": ["alt+t"], "app.message.followUp": followUp });
