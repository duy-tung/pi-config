...
const uiPackages = ["pi-open-tui", "compact-adviser"];
const definitions = {
  main: {
    runtime: "current", provider: "openai-codex", model: "gpt-6-astra", thinking: "high",
    theme: "rose-pine-moon", packages: [...common, "@tintinweb/pi-subagents", "pi-lens", ...uiPackages],
  },
  goal: {
    runtime: "compat", provider: "openai-codex", model: "gpt-6-astra", thinking: "high",
    theme: "rose-pine", packages: [...common, "@tintinweb/pi-subagents", "pi-lens", "pi-goal-x", "pi-workspace-history", ...uiPackages],
  },
  background: {
    runtime: "compat", provider: "openai-codex", model: "gpt-6-astra", thinking: "high",
    theme: "rose-pine", packages: [...common.filter((name) => name !== "@gotgenes/pi-anthropic-auth"), "pi-background-tasks", ...uiPackages],
  },
  advisor: {
    runtime: "current", provider: "openai-codex", model: "gpt-5.6-sol", thinking: "high",
    theme: "rose-pine", packages: [...common, "pi-lens", "pi-advisor-flow", ...uiPackages],
  },
};
...
export function buildConfiguration({ root, agentDir, binDir, nodePath, platform = process.platform, home, repoDir, shellPath }) {
  ...
  const protectedPaths = [
    ...
    join(home, ".pi", "agent", "auth.json"), join(home, ".pi", "agent", "compact-adviser.json"),
    join(agentDir, "auth.json"), join(root, "profiles", "*", "auth.json"),
    join(agentDir, "compact-adviser.json"), join(root, "profiles", "*", "compact-adviser.json"),
    join(root, "secrets", "*.env"), firecrawlCredentials,
  ];
  const policy = {
    permission: {
      ...
      path: {
        ...
        "**/routing.json": "deny", "**/routing-capabilities.json": "deny", "**/routing-state/**": "deny", "**/routing-writers/**": "deny",
      },
  ...
  for (const [name, profile] of Object.entries(profiles)) {
    ...
    const settings = {
      ...
      defaultProjectTrust: "ask", packages: profile.packages.map((pkg) => join(root, "runtimes", profile.runtime, "node_modules", pkg)),
      ...
      extensions: [`-${join(dir, "extensions", "statusline.ts")}`, join(targetAssets, "extensions", "rose-pine-palette.ts")],
    ...
    if (name === "main" || name === "goal") settings.extensions.push(join(targetAssets, "extensions", "pi-dispatch-router", "index.ts"));
    if (name === "goal") settings.workspaceHistory = {
    ...
    // Backport the real Astra definition from the pinned Pi 0.86.0 catalog.
    modelOverrides["gpt-6-astra"] = { contextWindow: 872000 };
    const providerModels = { modelOverrides };
    if (profile.runtime === "compat") providerModels.models = [readJson("astra-compat")];
    ...
    for (const [file, asset] of [
      ["subagents.json", "subagents"], ["advisor.json", "advisor"],
      ["pi-goal-x-settings.json", "goal"], ["open-tui.json", "open-tui"],
      ["routing.json", "routing"], ["routing-capabilities.json", "routing-capabilities"],
    ]) addJson(join(dir, file), readJson(asset));
    const routingFile = result.find(entry => entry.path === join(dir, 'routing.json'));
    routingFile.content = JSON.stringify({ ...readJson('routing'), writerLocksDir: join(root, 'state', 'routing-writers') }, null, 2) + '\n';
    addJson(join(dir, "compact-adviser.json"), {
      version: 1, mode: "off", minContextTokens: 40000, autoAcknowledged: false, logRequests: false,
    });
    if (name === "main") addJson(join(dir, "pi-usage.json"), { codexFastMode: true });
    addText(join(dir, "AGENTS.md"), readText("AGENTS.md"));
    for (const role of roleNames) addText(join(dir, "agents", `${role}.md`), readText(`roles/${role}.md`));
  }
