...
export function reconcileConfigFile({root, file, content, mode = 0o600, recorded, backup = () => {}}) {
  ...
  if (plan.content !== undefined) {
    ...
    bytes = Buffer.from(plan.content);
    writeAtomic(file, bytes, mode);
  }
  if (base !== content) writeAtomic(baseFile, Buffer.from(content), 0o600);
  return {hash: sha256(bytes), written: plan.content !== undefined, changes: plan.changes, conflicts: plan.conflicts, additive: plan.additive === true};
}
