...
export function reconcileResources({root, agentDir, binDir, state, wanted, log = console.log}) {
  const profiles = [agentDir, ...['goal','background','advisor'].map(name => path.join(root,'profiles',name))];
  const scopes = [...['assets','vendor','bin','config'].map(name => path.join(root,name)), ...profiles, binDir];
  const references = [];
  for (const directory of profiles) {
    const settingsFile = path.join(directory,'settings.json');
    if (!fs.existsSync(settingsFile)) continue;
    const settings = readJson(settingsFile);
    for (const entry of [...(settings.extensions ?? []), ...(settings.themes ?? []), ...(settings.skills ?? [])]) {
