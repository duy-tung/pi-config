...
import {RETIRED_DENY, deepEqual, reconcileJson, reconcileRole} from './merge.mjs';
...
/**
 * Cập nhật một file cấu hình JSON hoặc file role: gộp mặc định mới với file hiện tại theo base (xem lib/merge.mjs), chỉ ghi khi
 * kết quả khác file hiện tại (backup trước khi ghi đè), rồi lưu mặc định mới làm base. recorded là checksum
 * installer ghi lần trước, dùng khi chưa có base. Giữ nguyên (preserved) file không đọc được (JSON hỏng, frontmatter
 * không phải dạng key: value) và file có
...
export function reconcileConfigFile({root, file, content, mode = 0o600, recorded, backup = () => {}}) {
  ...
  const plan = path.extname(file) === '.md'
    ? reconcileRole({next: content, current: current?.toString('utf8'), base, unedited})
    : reconcileJson({next: content, current: current?.toString('utf8'), base, unedited, settings: path.basename(file) === 'settings.json'});
