...
export function pruneInactiveConfiguration({root, agentDir, state, desiredFiles, log = console.log}) {
  ...
  const obsoleteAssets = [
    ...['routing.json','routing-capabilities.json'].map(name => path.join(root,'assets/configs',name)),
    ...['researcher-glm','worker-glm','debugger-glm'].map(name => path.join(root,'assets/roles',name+'.md')),
  ];
  let archive;
