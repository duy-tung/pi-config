...
import { classifyWithFallback, type Complete } from "./lib/classifier.ts";
...
import { type EvalCase, formatReport, runEval } from "./lib/eval.ts";
...
export default function piAutoMode(pi: ExtensionAPI) {
  ...
  async function loadFacts(cwd: string): Promise<void> {
    ...
    if (top) {
      ...
      lines.push(remotes.length ? `Trusted remotes (at session start): ${remotes.join("; ")}` : "The trusted repository has no remotes.");
    } else {
  ...
  async function runClassifier(ctx: ExtensionContext, call: ToolCall, toolCallId: string | undefined, notes: string[]) {
    const session = ctx.model;
    const model = demoted ? session : resolveModel(ctx, config.model);
    if (!model) return { kind: "unavailable" as const, reason: "no model is configured for the classifier" };
    ...
    try {
      const { result, fellBack, primaryReason } = await classifyWithFallback({
        ...
        stage2Reasoning: config.stage2Reasoning, signal: ctx.signal,
      }, fallback);
  ...
  async function gate(ctx: ExtensionContext, call: ToolCall, toolCallId?: string): Promise<{ block: true; reason: string } | undefined> {
    ...
    const result = await runClassifier(ctx, call, toolCallId, decision.notes);
    if (result.kind === "allow") return allowed(call, `classifier stage ${result.stage}`);
  ...
  function modelLabel(): string {
    ...
    return `${config.model ?? "session model"}${stage2}`;
  }
  ...
  pi.registerCommand("auto-mode", {
    description: "Auto mode status, defaults, dry run and eval: /auto-mode [status|defaults|test <command>|eval [provider/model]]",
    getArgumentCompletions: (prefix) => ["status", "defaults", "test ", "eval"].filter((item) => item.startsWith(prefix)).map((value) => ({ value, label: value.trim() })),
    handler: async (args, ctx) => {
      const [sub, ...rest] = args.trim().split(/\s+/u);
      if (sub === "eval") {
        // Chạy bộ đánh giá có nhãn qua bộ phân loại thật (tốn quota); không dùng luật allow/ask/deny.
        const file = selfDir ? path.join(selfDir, "eval", "cases.json") : undefined;
        if (!file || !fs.existsSync(file)) {
          notify(ctx, "Eval cases are missing", "error");
          return;
        }
        const spec = rest[0] ?? config.model;
        ...
        notify(ctx, `Running ${cases.length} eval cases with ${model.provider}/${model.id}…`, "info");
        const outcomes = await runEval(cases, {
          slots: resolveSlots({ ...config, deny: [] }, []), complete: makeComplete(ctx, model, model, `pi-auto-mode-eval:${Date.now()}`),
          timeoutMs: config.timeoutMs, stage2Reasoning: config.stage2Reasoning,
          decide: (call) => decide(call, evalContext), skipTools: SAFE_TOOLS, concurrency: 3,
        });
        const report = formatReport(outcomes, `${model.provider}/${model.id}`);
      ...
      if (sub === "test") {
        ...
        const result = await runClassifier(ctx, call, undefined, decision.notes);
        const summary = result.kind === "allow" ? `allow (stage ${result.stage})`
          : result.kind === "block" ? `block (stage ${result.stage}) — ${result.rule ? `[${result.rule}] ` : ""}${result.reason}`
            : `unavailable — ${result.reason}`;
        notify(ctx, `Classifier: ${summary}`, result.kind === "allow" ? "info" : "warning");
        return;
      }
      const set = rules();
      notify(ctx, [
